...
function DashBoardRecentTransaction({ transactions }: { transactions: Transaction[] }) {
  ...
  return (
  <div className="bg-slate-700 rounded-2xl p-2 h-full grid grid-rows-[auto_auto_1fr] overflow-hidden">
    ...
    {recentTransactions.length === 0 ? (
      ...
      <div className="grid grid-rows-3 gap-2 h-full mt-3">
        {recentTransactions.map((transaction) => (
          <div
            key={transaction.id}
            className="grid grid-cols-3 mx-2 rounded-xl px-2 py-2.5 bg-slate-800 border mb-5"
          >
